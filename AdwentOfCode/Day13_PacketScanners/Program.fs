...
//that (with range 2), a third layer which begins at depth 4 (with range 4), and a fourth layer which begins 
...
//[ ]             [ ]     [ ]
//                [ ]     [ ]
//Within each layer, a security scanner moves back and forth within its range. Each security scanner starts 
//at the top and moves down until it reaches the bottom, then moves up until it reaches the top, and repeats.
...
//[ ]             [ ]     [ ]
//                [S]     [S]
//Your plan is to hitch a ride on a packet about to move through the firewall. The packet will travel along the 
//top of each layer, and it moves at one layer per picosecond. Each picosecond, the packet moves one layer 
//forward (its first move takes it into layer 0), and then the scanners move one step. If there is a scanner 
//at the top of the layer as your packet enters it, you are caught. (If a scanner moves into the top of its 
//layer while you are there, you are not caught: it doesn't have time to notice you before you leave.) If you 
//were to do this in the configuration above, marking your current position with parentheses, your passage 
//through the firewall would look like this:
...
//the top when you entered it. You are not caught in layer 1, since the scanner moved into the top of the layer once you were already there.

//The severity of getting caught on a layer is equal to its depth multiplied by its range. (Ignore layers in which 
//you do not get caught.) The severity of the whole trip is the sum of these values. In the example above, 
...
let part1 = 
    ...
    let layers = [|for _ in 0 .. maxValue -> 0|]
    let mutable result = 0
        
    for elem in lines do
        let values = elem.Split [|':'|]
        let index = Convert.ToInt32 (values.[0].Trim())
        let maxValue = Convert.ToInt32 (values.[1].Trim())
        layers.[index] <- maxValue
        
    for i = 0 to Array.length layers - 1 do
        if layers.[i] <> 0 then
            let moveRange = layers.[i] * 2 - 2
            if i % moveRange = 0 then
                result <- result + i * layers.[i]
...
let part2 = 
    ...
    let willIGetCaught delay (layers : int[]) = 
        ...
        while caught <> true && index < Array.length layers do
            if layers.[index] <> 0 then
                let moveRange = layers.[index] * 2 - 2
                caught <- (index + delay) % moveRange = 0
            index <- index + 1

        caught

    let mutable delay = 0
    while willIGetCaught delay layers do
       delay <- delay + 1

    printfn "Part2 result: %i" delay
