...
//position. It then steps forward through the circular buffer some number of steps (your puzzle input) 
//before inserting the first new value, 1, after the value it stopped on. The inserted value becomes 
//the current position. Then, it steps forward from there the same number of steps, and wherever 
...
//evolve like this (using parentheses to mark the current position after each iteration of the algorithm):
...
let part1 = 
    ...
    let performStep (array: int[], step : int, currentIndex : int, value : int) = 
        let arrayLength = array.Length
        let newValueIndex = (currentIndex + step) % arrayLength + 1
        let newArray = [|for i in 0..arrayLength -> 0|]

        for i = 0 to newValueIndex - 1 do
            newArray.[i] <- array.[i]

        newArray.[newValueIndex] <- value

        for i = newValueIndex + 1 to arrayLength do
            newArray.[i] <- array.[i - 1]

        (newArray, newValueIndex)
        
    for i in 1..2017 do
        let (newArray, newCurrentIndex) = performStep(array, step, currentIndex, i)
        currentIndex <- newCurrentIndex
        array <- newArray

    printfn "Part1 result: %i" array.[currentIndex + 1]
...
//The bad news is that while you were determining this, the spinlock has just finished 
//inserting its fifty millionth value (50000000).
...
let part2 = 
    ...
    let performStep (step : int, currentIndex : int, value : int, valueAfterZero : int) = 
        let newValueIndex = (currentIndex + step) % value + 1
        if newValueIndex - 1 = 0 then
            (value, newValueIndex)
        else
            (valueAfterZero, newValueIndex)
        
    for i in 1..50_000_000 do
        let (newValueAfterZero, newCurrentIndex) = performStep(step, currentIndex, i, valueAfterZero)
        currentIndex <- newCurrentIndex
        valueAfterZero <- newValueAfterZero

    printfn "Part2 result: %i" valueAfterZero
