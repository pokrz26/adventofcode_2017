...
//Here, you encounter a pair of dueling generators. The generators, called generator 
//A and generator B, are trying to agree on a sequence of numbers. However, one of 
//them is malfunctioning, and so the sequences don't always match.

//As they do this, a judge waits for each of them to generate its next value, compares
...
//For example, suppose that for starting values, generator A uses 65, while generator 
//B uses 8921. Then, the first five pairs of generated values are:
...
//1110001101001010. Because of this one match, after processing these five pairs, 
...
let part1 =
    //let mutable prevValueA = 65
    //let mutable prevValueB = 8921
    let mutable prevValueA = 116
    let mutable prevValueB = 299
    let factorA = 16807
    let factorB = 48271
    ...
    for i = 1 to 40000000 do
        prevValueA <- (int32)((int64)prevValueA * (int64)factorA % (int64)divider)
        prevValueB <- (int32)((int64)prevValueB * (int64)factorB % (int64)divider)

        if (prevValueA &&& 0x0000FFFF) = (prevValueB &&& 0x0000FFFF) then
            result <- result + 1
...
//In the interest of trying to align a little better, the generators get more picky about 
//the numbers they actually give to the judge.

//They still generate values in the same way, but now they only hand a value to the judge 
...
let part2 = 
    ...
    let rec getNewValue (prevValue : int32, factor : int32,  divider : int32, resultDivider : int32) : int32 =
        let newValue = (int32)((int64)prevValue * (int64)factor % (int64)divider)
        if newValue % resultDivider <> 0 then
            getNewValue(newValue, factor, divider, resultDivider)
        else
            newValue

    for i = 1 to 5000000 do
        prevValueA <- getNewValue(prevValueA, factorA, divider, 4)
        prevValueB <- getNewValue(prevValueB, factorB, divider, 8)

        if (prevValueA &&& 0x0000FFFF) = (prevValueB &&& 0x0000FFFF) then
            result <- result + 1

    printfn "Part2 result: %i" result
