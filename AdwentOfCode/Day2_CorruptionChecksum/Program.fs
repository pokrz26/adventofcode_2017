...
//The spreadsheet consists of rows of apparently-random numbers. To make sure the recovery process is on the right track, they need you to calculate 
//the spreadsheet's checksum. For each row, determine the difference between the largest value and the smallest value; the checksum is the sum of all 
...
let part1 = 
    ...
    for line in input do
        let numbers = line.Split([|'\t'; ' '|], StringSplitOptions.None) |> Array.map int32
        let min = Seq.min numbers
        let max = Seq.max numbers
        result <- result + max - min

    printfn "part 1 result: %i" result
...
let part2 =
    ...
    for line in input do
        ...
        for i = 0 to Array.length numbers - 1 do
            for j = 0 to Array.length numbers - 1 do
                if i <> j && numbers.[i] % numbers.[j] = 0 then
                    result <- result + numbers.[i] / numbers.[j]

    printfn "part 2 result: %i" result
